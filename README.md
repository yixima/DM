...
内容がどれだけ良くても、**送信ドメインの認証設定が欠けていれば大半は迷惑メールに入ります。**
Gmail は1日5,000通を超える送信者に SPF・DKIM・DMARC の設定を必須とし、迷惑メール報告率を
0.30%未満（推奨0.1%未満）に保つことを求めています。
（[Gmail 送信者ガイドライン](https://support.google.com/a/answer/81126)）

### 設定できているかを実測する

```bash
python -m dm.cli doctor --dns
```

実際に DNS を引いて、次を1つずつ判定します。

| 検査 | 落ちていると起きること |
|---|---|
| SPF | 送信サーバが「なりすまし」と判定される |
| DKIM | 本文の改ざん検知ができず、認証が通らない |
| DMARC | 認証失敗時の扱いが未宣言で、受信側が保守的に判断する |
| 逆引き（PTR） | 素性の分からないサーバとして扱われる |
| From ドメインの整合 | From・返信先・配信停止先がバラけると認証が通りにくい |
| 1日あたりの上限 | 5,000通を超えると要求水準が上がる |

**「確認不可」と「未設定」は区別して表示されます。** DNS に到達できなかっただけの状態を
「設定されていない」とは報告しません。DKIM のセレクタが独自の場合は
`--dkim-selector <名前>` で指定してください。

### システム側で既に実装している対策

| 対策 | 内容 |
|---|---|
| ワンクリック配信停止 | `List-Unsubscribe` と `List-Unsubscribe-Post` を全通に付与（RFC 8058） |
| 本文はテキストのみ | HTML を使わず、画像・追跡ピクセルも入れない |
| 送信ペースの抑制 | 1通ごとに2秒＋ゆらぎ、毎時120通まで |
| 1日あたりの上限 | 既定1,000通。到達すると自動で打ち切り、翌日に繰り越す |
| バウンスの自動除外 | 恒久エラーは即除外、一時エラーは3回で除外 |
...
- **Postmaster Tools への登録**（Google）。迷惑メール報告率は、ここでしか見えません。
- **ウォームアップ**。新しい送信ドメインでいきなり大量に送らないこと。
- **反応のない宛先を送り続けない**。開封も返信もない相手への継続送信は報告率を押し上げます。
